...
import { Send, Bot, User, Zap, Target, TrendingUp, Activity, Bookmark, Clock, X } from "lucide-react";
...
const AICoach = () => {
  ...
  const { messages, loading, sendMessage, stopGeneration } = useAIChat();
      ...
      <div className="flex flex-col h-[calc(100vh-160px)] pb-20">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map(message => <div key={message.id} className={`flex gap-3 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {message.role === 'assistant' && <Avatar className="h-8 w-8 border border-primary/50">
                  <AvatarFallback className="bg-gradient-primary text-primary-foreground">
              ...
              <Card className={`max-w-[80%] ${message.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted border-primary/20'}`}>
                <CardContent className="p-3">
                  ...
                  {message.role === 'assistant' && profile?.auto_save_bets !== false && <Button onClick={() => setSaveBetOpen(true)} size="sm" className="mt-3 bg-gradient-neon hover:shadow-glow font-sports text-accent-foreground">
              ...
              {message.role === 'user' && <Avatar className="h-8 w-8 border border-primary/50">
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <Input 
                ...
                className="pr-12 bg-muted/50 border-primary/20 focus:border-primary font-normal text-base" 
              />
            </div>
            <Button onClick={handleSendMessage} disabled={!inputValue.trim() || loading} variant="gaming" size="icon" className="shrink-0">
              <Send className="h-4 w-4" />
            </Button>
          </div>
          <div className="text-xs text-muted-foreground mt-2 text-center font-normal">
